...
for size in [4,6,8] :
    X_scaled =np.array([resize(image,(size,size)) for image in X])
    print("Size of Rescaled image : ",X_scaled[0].shape)
    for dev_size , test_size in all_test_dev_combination :
        # Data Splitting -- to create train and test sets
        X_train, X_test,X_dev, y_train, y_test,y_dev = split_data(X_scaled , y,test_size = test_size,dev_size = dev_size)
